...
function UsersTab() {
  ...
  const toggleBlock = useMutation({
    mutationFn: ({ id, blocked }: { id: string; blocked: boolean }) => AdminAPI.toggleUserBlock(id, blocked),
...
function ProductsTab() {
  ...
  const toggleHidden = useMutation({
    mutationFn: ({ id, hidden }: { id: string; hidden: boolean }) => AdminAPI.toggleProductVisibility(id, hidden),
          ...
          {isLoading ? (
            ...
            items.map((p) => (
              <tr key={p._id} className="border-t border-border">
                ...
                <td className="px-6 py-3 text-muted-foreground">{typeof p.seller === "object" ? p.seller?.name ?? "—" : "—"}</td>
...
function OrdersTab() {
  ...
  const items: AdminOrder[] = data?.items ?? [];
  const totalPages = data?.totalPages ?? 1;

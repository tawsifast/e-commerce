...
import { AuthAPI, getApiErrorMessage, OrdersAPI, WishlistAPI } from "@/lib/api";
...
export const Route = createFileRoute("/dashboard/buyer")({
  head: () => ({
    meta: [
      { title: "My Dashboard — Marketa" },
      { name: "description", content: "Manage your orders, wishlist, and profile on Marketa." },
  ...
  if (ordersQ.isError) {
    return (
      <EmptyState
        ...
        body={`Make sure your API is running at ${import.meta.env.VITE_API_BASE_URL ?? "http://localhost:5000/api"}.`}
      />
    );
  }
  ...
  if (items.length === 0) {
    return (
      <EmptyState
        ...
        cta={<Link to="/products"><Button className="bg-gradient-hero text-primary-foreground hover:opacity-90">Browse products</Button></Link>}
      />
    );
  }
  ...
  if (items.length === 0) {
    return (
      <EmptyState
        ...
        cta={<Link to="/products"><Button className="bg-gradient-hero text-primary-foreground hover:opacity-90">Discover products</Button></Link>}
      />
    );
  }
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {items.map((p, i) => (
        ...
        >
          <Link to="/products/$id" params={{ id: p._id }} className="h-24 w-24 shrink-0 overflow-hidden rounded-lg bg-muted">
            {p.images?.[0] && <img src={p.images[0]} alt={p.title} className="h-full w-full object-cover" />}
          </Link>
          <div className="flex flex-1 flex-col">
            <div className="flex items-start justify-between gap-2">
              <Link to="/products/$id" params={{ id: p._id }} className="line-clamp-2 text-sm font-medium hover:underline">
                {p.title}
              </Link>
              <button

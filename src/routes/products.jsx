...
export const Route = createFileRoute("/products")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse every product on Marketa. Filter by category, brand, price, and rating." },
      { property: "og:title", content: "All Products — Marketa" },
      { property: "og:description", content: "Browse every product on Marketa." },
    ],
  }),
        ...
        <div>
          {query.isLoading ? (
            ...
            <div className="rounded-xl border border-destructive/40 bg-destructive/5 p-8 text-center text-sm text-destructive">
              Couldn't load products. Check your API is running at <code>{import.meta.env.VITE_API_BASE_URL ?? "http://localhost:5000/api"}</code>.
            </div>
